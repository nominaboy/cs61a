def couple(s, t):
    ...
    assert len(s) == len(t)
    "*** YOUR CODE HERE ***"
    return [list((s[i], t[i])) for i in range(0, len(s))]
...
def coords(fn, seq, lower, upper):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [list((seq[i], fn(seq[i]))) for i in range(0, len(seq)) if fn(seq[i]) >= lower and fn(seq[i]) <= upper]
...
def add_trees(t1, t2):
    ...
    i = 0
    """can be replaced by 
        result_branches = [add_trees(br1, br2) for br1, br2 in zip(branches(t1), branches(t2)]
        ...
        i = len(result_branches)"""
    while i < len(branches(t1)) and i < len(branches(t2)):
        br1, br2 = branches(t1)[i], branches(t2)[i]
        new_branch = add_trees(br1, br2)
