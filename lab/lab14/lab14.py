...
def num_splits(s, d):
    ...
    def split(s, n):
        if not s:  # no left elements
            ...
            first, rest = s[0], s[1:]
            return split(rest, n+first) + split(rest, n-first)  # calculate all the possibilities (left pool OR right pool) 
    return split(s, 0) // 2   # assume the start diff is zero
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check_obj):
        if self.holder == check_obj.payable_to:
            if not check_obj.deposited:
                check_obj.deposited = True
                return check_obj.balance
            else:
