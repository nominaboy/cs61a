    ...
    def nearest_bee(self, beehive):
        ...
        def nb_helper(place, counter):
            ...
            if place.bees and (self.min_range <= i <= self.max_range):
                ...
                i += 1
                return nb_helper(place.entrance, i)
        return nb_helper(self.place, i)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if cur_place.bees:
            copy_beelist = list(cur_place.bees)
            for each_bee in copy_beelist:
                if self.armor <= 0:
                    Insect.reduce_armor(each_bee, amount + self.damage)
                else:
                    Insect.reduce_armor(each_bee, amount)
        # END Problem 5
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        self.first_queenant = QueenAnt.first_queenant
        if self.first_queenant == True:
            QueenAnt.first_queenant = False
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.first_queenant == False:
            self.reduce_armor(self.armor)
        else:
            ScubaThrower.action(self, gamestate)
            cur_place = self.place.exit
            while cur_place:
                if cur_place.ant and cur_place.ant.damage_double == False:
                    cur_place.ant.damage *= 2
                    cur_place.ant.damage_double = True            
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        Ant.reduce_armor(self, amount)
        if self.first_queenant == True and self.armor <= 0:
            bees_win()
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while cur_place and (cur_place is not beehive):
            if cur_place.ant and (cur_place.ant is not self):
                return_value[cur_place.ant] = cur_distance
            for i in cur_place.bees:
                return_value[i] = cur_distance 
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        cur_damage = self.damage - 0.05 * self.insects_shot
        final_damage = cur_damage - distance * 0.2
        return final_damage
        # END Problem Optional 5
