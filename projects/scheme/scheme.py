...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        return None
    while True:
        result_value = scheme_eval(expressions.first, env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        result_value = scheme_eval(rest.first, env)
        env.define(target, result_value)
        return target
        # END PROBLEM 5
        ...
        t_proc = do_lambda_form(Pair(formals, body), env)
        env.define(name, t_proc)
        return name
        # END PROBLEM 9
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    rest_part = expressions.rest
    return LambdaProcedure(formals, rest_part, env)
    # END PROBLEM 8
