...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while roll_counter < num_rolls:
        six_sided_value = dice()
        result = result + six_sided_value
        if six_sided_value == 1:
            sign_for_pigout = 1
        roll_counter += 1
    if sign_for_pigout == 1:
        result = 1
    return result
    # END PROBLEM 1
...
def free_bacon(score):
    ...
    "*** YOUR CODE HERE ***"
    """pi_trim_counter = 0
    while pi_trim_counter < 100-score:
        pi = pi // 10
        pi_trim_counter += 1"""
    pi //= pow(10, 100-score)
    # END PROBLEM 2
...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        return False
    while counter <= max(player_score, opponent_score):
        if player_score % counter == 0 and opponent_score % counter == 0:
            if counter >= 10:
                return True
        counter += 1
    return False"""
    counter = max(player_score, opponent_score)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while True:
        if who == 0:
            player0_num_dices = strategy0(score0, score1)
            score0 += take_turn(player0_num_dices, score1, dice)
            ...
            if not extra_turn(score0, score1):
                who = other(who)
        if who == 1:
            player1_num_dices = strategy1(score1, score0)
            score1 += take_turn(player1_num_dices, score0, dice)
            say = say(score0, score1)
            if score1 >= goal:
                break
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def comment(score0, score1):
        if who == 0:
            gap_score = score0 - last_score
            if gap_score > running_high:
                print(gap_score, 'point(s)! The most yet for Player', who)
                return announce_highest(who, score0, gap_score)
            else:
                return announce_highest(who, score0, running_high)
        if who == 1:
            gap_score = score1 - last_score
            if gap_score > running_high:
                print(gap_score, 'point(s)! The most yet for Player', who)
                return announce_highest(who, score1, gap_score)
            else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    counter = 1
    while n <= 10:
        cur_value = make_averaged(roll_dice, trials_count)(n, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    value_add = free_bacon(opponent_score)
    value_after = score + value_add
    if extra_turn(value_after, opponent_score):
        return 0
    else:
